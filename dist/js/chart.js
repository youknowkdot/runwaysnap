...
window.RSChart = {
  render(svgEl, labels, values) {
    ...
    if (n < 2) {
      svgEl.innerHTML =
        '<text x="50%" y="50%" text-anchor="middle" fill="#8b949e" font-size="14">Add inputs to see your runway</text>';
      svgEl.setAttribute("viewBox", `0 0 ${w} ${h}`);
      return;
    }

    const xAt = (i) => pad.l + (i / (n - 1)) * iw;
    const yAt = (v) => pad.t + ih - ((v - minV) / (maxV - minV)) * ih;
    ...
    for (let t = 0; t <= ticks; t++) {
      ...
      grid += `<text x="${pad.l - 8}" y="${y + 4}" text-anchor="end" fill="#8b949e" font-size="11" font-family="system-ui,sans-serif">${label}</text>`;
    }
    ...
    for (let i = 0; i < n; i += step) {
      xLabels += `<text x="${xAt(i)}" y="${h - 10}" text-anchor="middle" fill="#8b949e" font-size="11" font-family="system-ui,sans-serif">${labels[i]}</text>`;
    }
    if ((n - 1) % step !== 0) {
      const i = n - 1;
      xLabels += `<text x="${xAt(i)}" y="${h - 10}" text-anchor="middle" fill="#8b949e" font-size="11" font-family="system-ui,sans-serif">${labels[i]}</text>`;
    ...
    svgEl.innerHTML = `
      ...
      <circle cx="${xAt(0)}" cy="${yAt(values[0])}" r="4" fill="#1f6b4a"/>
      <circle cx="${xAt(n - 1)}" cy="${yAt(values[n - 1])}" r="4" fill="${values[n - 1] <= 0 ? "#b91c1c" : "#1f6b4a"}"/>
      ${xLabels}
    `;
  },
};
